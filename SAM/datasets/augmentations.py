...
class GenderTransformer(object):

	def __init__(self, target_gender):
    self.target_gender = target_gender

	def __call__(self, img):
		img = self.add_gender_channel(img)
		return img

	def add_gender_channel(self, img):
		target_gender = self.__get_target_gender()
		target_gender = int(target_gender)  # normalize aging amount to be in range [-1,1]
		img = torch.cat((img, target_gender * torch.ones((1, img.shape[1], img.shape[2]))))
		return img
	def __get_target_gender(self):
		if self.target_gender == "uniform_random":
